\documentclass[letterpaperj]{afla}
...
To use the \verb`afla` class in your document, start the preamble with \verb`\documentclass` \verb`{afla}`. Two paper size options are defined: \verb`\documentclass[letterpaper]` \verb`{afla}` will use US letter paper size and \verb`\documentclass[b5j]{afla}` will use JIS B5 paper size (specified in some years for publication through \textit{Asia-Pacific Linguistics}). If no paper size option is specified, \verb`\documentclass{afla}` will default to US letter paper.
